<?php 
    ...
    $current_options = array("Event", "Raid Boss");

    if(isset($_POST["submit"])){
        if(edit_event($_POST) >= 0){
            echo "
            <script>
        ...
        <div class="container">
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="mb-2 pt-3 row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-select form-select-md mb-3" aria-label=".form-select-lg example" name="current" required>
                            <?php 
                                foreach($current_options as $copti){
                                    if($data['current'] == $copti){
                                        echo "<option selected value='$copti'>$copti</option>";
                                    }
                                    else{
                                        echo "<option value='$copti'>$copti</option>";
                                    }
                                }
                            ?>
                        </select>
                ...
                <div class="mb-3 pt-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="Judul" name="zidan" value="<?php echo $data['judul']; ?>" required>
                    </div>
                </div>
                <div class="mb-3 pt-3 row">
                    <label for="Tanggal" class="col-sm-2 col-form-label">Tanggal</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="Tanggal" name="tanggal" value="<?php echo $data['tanggal']; ?>" required>
                    </div>
                </div>
                <button type="submit" name="submit" class="btn btn-primary mb-3">Edit Event</button>
